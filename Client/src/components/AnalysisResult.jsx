...
function AnalysisResult({ result }) {
  ...
  const getTypeIcon = () => {
    const type = result?.inputType?.toLowerCase() || '';
    
    if (type.includes('url')) return <LinkIcon size={20} />;
    if (type.includes('domain')) return <Globe size={20} />;
    if (type.includes('hash')) return <Hash size={20} />;
    if (type.includes('ip')) return <Wifi size={20} />;
    if (type.includes('file')) return <FileText size={20} />;
